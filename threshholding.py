...
Original file is located at
    https://colab.research.google.com/drive/1W_PIOJF1yM8a4_OMl6mP1HnrUSiqM8o1
"""
...
IMAGE_DIR = "/content/thre_imgs"
ANNOTATION_DIR = "/content/drive/My Drive/whole_labels"

# making whole json_file

p = 'thre_whole_patches'
os.makedirs(p, exist_ok=True)

pa = 'thre_whole_patch_coco_octa.json'
...
out_path = "/content/thre_whole_patch_coco_octa.json"
...
for (path, dir, files) in os.walk(IMAGE_DIR):
  for i, filename in enumerate(files):
    ...
    for j in range(len(patches)):     
      ...
      path_save_name = "thre_whole_patches/" + save_name 
      re.save(path_save_name)
...
whole_file = "thre_whole_patch_coco_octa.json"
...
for images in ddd["images"]:
  k = "/content/thre_whole_patches/" + images["file_name"]
  name_list.append(k)
...
for imageName in glob.glob('/content/thre_whole_patches/*png'):
...
for (path, dir, files) in os.walk("/content/drive/My Drive/new_imgs"):
  for file in files:
    if (file in whole.keys()):
      ...
      # save results
      mm = im_path.split("/")[-1]
      path_save_name = "/content/dsad/" + mm
      re.save(path_save_name)
      plt.imshow(img)
      print(im_path)
...
cfg.MODEL.WEIGHTS = os.path.join(cfg.OUTPUT_DIR, "model_final.pth")
# cfg.DATASETS.TEST = ("dataset_train00", )
...
cfg = get_cfg()
cfg.merge_from_file(model_zoo.get_config_file("COCO-Detection/faster_rcnn_X_101_32x8d_FPN_3x.yaml"))
cfg.DATASETS.TRAIN = ("dataset_val11",)
cfg.DATASETS.TEST = ("dataset_train11",)
cfg.DATALOADER.NUM_WORKERS = 1
cfg.MODEL.WEIGHTS = model_zoo.get_checkpoint_url("COCO-Detection/faster_rcnn_X_101_32x8d_FPN_3x.yaml")  # Let training initialize from model zoo
cfg.SOLVER.IMS_PER_BATCH = 4
cfg.SOLVER.BASE_LR = 0.00025
...
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.7   # set the testing threshold for this model
predictor = DefaultPredictor(cfg)
test_metadata = MetadataCatalog.get("dataset_train11",)
print("done")
...
for (path, dir, files) in os.walk("/content/drive/My Drive/imgs"):
  for file in files:
    if (file in whole.keys()):
      ...
      # save results
      mm = im_path.split("/")[-1]
      path_save_name = "/content/drive/My Drive/thre_second_20/" + mm
      re.save(path_save_name)
      plt.imshow(img)
      print(im_path)
...
evaluator = COCOEvaluator("dataset_train11", cfg, False, output_dir="./output/")
val_loader = build_detection_test_loader(cfg, "dataset_train11")
inference_on_dataset(trainer.model, val_loader, evaluator)
...
cfg = get_cfg()
cfg.merge_from_file(model_zoo.get_config_file("COCO-Detection/faster_rcnn_X_101_32x8d_FPN_3x.yaml"))
cfg.DATASETS.TRAIN = ("dataset_val22",)
cfg.DATASETS.TEST = ("dataset_train22",)
cfg.DATALOADER.NUM_WORKERS = 1
cfg.MODEL.WEIGHTS = model_zoo.get_checkpoint_url("COCO-Detection/faster_rcnn_X_101_32x8d_FPN_3x.yaml")  # Let training initialize from model zoo
cfg.SOLVER.IMS_PER_BATCH = 4
cfg.SOLVER.BASE_LR = 0.00025
...
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.7   # set the testing threshold for this model
predictor = DefaultPredictor(cfg)
test_metadata = MetadataCatalog.get("dataset_train12",)
print("done")
...
for (path, dir, files) in os.walk("/content/drive/My Drive/imgs"):
  for file in files:
    if (file in whole.keys()):
      ...
      # save results
      mm = im_path.split("/")[-1]
      path_save_name = "/content/drive/My Drive/thre_third_25/" + mm
      re.save(path_save_name)
      plt.imshow(img)
      print(im_path)
...
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.6   # set the testing threshold for this model
predictor = DefaultPredictor(cfg)
test_metadata = MetadataCatalog.get("dataset_train22",)
